...
<div class="mb-8 animate-fade-in-up">
    <div class="flex items-center gap-3">
        <a href="{{ route('karang-taruna.warga.show', $warga) }}" class="w-10 h-10 bg-gray-200 rounded-full flex items-center justify-center hover:bg-gray-300 transition-colors">
            <i class="fas fa-arrow-left text-gray-700"></i>
        </a>
        <div>
            <h1 class="text-4xl font-black text-gray-900">Edit Warga</h1>
            <p class="text-gray-600 mt-1">Ubah data warga {{ $warga->nama }}</p>
        </div>
    </div>
</div>
...
<div class="bg-white rounded-3xl shadow-md border border-gray-200/50 p-8 animate-fade-in-up" style="animation-delay: 0.1s;">
    <form method="POST" action="{{ route('karang-taruna.warga.update', $warga) }}" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="flex gap-4 pt-6 border-t border-gray-200">
            <a href="{{ route('karang-taruna.warga.show', $warga) }}" class="flex-1 px-6 py-3 bg-gray-200 text-gray-900 font-semibold rounded-2xl hover:bg-gray-300 transition-all duration-300 text-center">
                Batal
            </a>
